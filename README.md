...
- ログイン後のログアウト
- RememberMe機能実装
### 【飲食店一覧ページ】 ゲスト含めた全ユーザー閲覧可能(権限によって一部サービス利用制限あり)
- 飲食店一覧閲覧
...
### 【飲食店詳細ページ】 ゲスト含めた全ユーザー閲覧可能(権限によって一部サービス利用制限あり)
#### **レビュー機能**
...
- 予約当日になったら09:00にリマインダーメールが届くように設定
...
- 個別ユーザー宛にメール送信機能(現在は常連ユーザー宛にクーポン付メール)
#### **shop_create画面**
...
### 新規登録時に認証メールによって本人確認を行う機能について
#### 全体に於ける注意点
- MustVerifyEmailインターフェースを実装しているため、登録したメールアドレスに送られる認証メールで承認しないと一部サービスの利用が制限されます。
#### 開発環境(docker)での確認メール送信注意点
- 確認メール受信確認場所 Mailhog http://localhost:8025/
#### 本番環境(AWS)での確認メール送信注意点
- Mailhogで確認する場合、Mailhog http://13.231.247.232:8025/
- SESにて実装。「ドメイン取得はなし」案件の為、送信元は自身のメールアドレスにて実装。その為DKIM等の処理が出来ず、送信された確認メールが迷惑メールBOXに入っている可能性あり。gmailドメインで実装可能。キャリアメール、outlookでは送信出来ない可能性あり。
